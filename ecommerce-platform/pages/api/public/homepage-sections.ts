...
const sanitizeContentItems = async (items: any[]) => {
    ...
    for (const item of items) {
        ...
        let populatedItemData: IProduct | ICategory | null = null; // Use imported types
        ...
        if (item.itemType === 'Product') {
            populatedItemData = await Product.findOne({ _id: item.itemId, isPublished: true }) // Add isPublished check
                ...
                .populate<{ category: ICategory | null }>('category', 'name slug isPublished') // Populate product's category and select its isPublished
                .lean();
        ...
        if (populatedItemData) {
            ...
            if (item.itemType === 'Product' && populatedItemData.category && typeof populatedItemData.category === 'object' && !(populatedItemData.category as ICategory).isPublished) {
            ...
            } else if (!finalImageUrl && item.itemType === 'Category' && (populatedItemData as ICategory).image?.url) { // For category image
                finalImageUrl = (populatedItemData as ICategory).image?.url;
            }
