...
export default function EditStaticPage() {
  ...
  useEffect(() => {
    if (pageTitle && !slugManuallyEdited && form.values.slug === generateSlug(originalSlug || '')) {
        ...
        // Or, if name changes and current slug IS the original slug derived from original name, then update.
        if (form.values.slug === originalSlug && generateSlug(pageTitle) !== originalSlug) {
            // If user changes title, and slug is still the original one, suggest update or auto-update if desired
            // form.setFieldValue('slug', generateSlug(pageTitle)); // This can be too aggressive
        }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pageTitle, slugManuallyEdited, originalSlug]);
