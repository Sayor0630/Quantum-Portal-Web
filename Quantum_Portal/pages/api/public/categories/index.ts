...
const filterUnpublishedChildren = (categories: any[]): any[] => {
    return categories
        .filter(category => category.isPublished) // Filter current level
        .map(category => {
            if (category.children && category.children.length > 0) {
                // Assuming children are populated objects, not just IDs
                const publishedChildren = category.children.filter((child: any) => child.isPublished);
                return { ...category, children: filterUnpublishedChildren(publishedChildren) };
            }
            return category;
        });
};
